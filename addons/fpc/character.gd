...
#region Wall Ride Export Group

@export_group("Wall Ride")
## Enable or disable wall riding.
@export var wall_ride_enabled : bool = true
## How long (seconds) the player can ride a wall before sliding off.
@export var wall_ride_duration : float = 2.0
## Slow downward drift speed while on wall.
@export var wall_slide_gravity : float = 2.0
## Horizontal push away from wall when wall jumping.
...
@export_group("Lean")
## Enable or disable leaning.
@export var lean_enabled : bool = true
## Maximum lean angle in degrees.
@export var lean_angle : float = 8.0
## How fast the lean transitions in and out.
@export var lean_speed : float = 8.0

#endregion

#region Stamina Export Group

@export_group("Stamina")
## Enable or disable the stamina system.
...
var wall_jumped : bool = false  # blocks re-grab until floor is touched again
...
func _physics_process(delta): # Most things happen here.
	...
	if not is_on_floor() and gravity and gravity_enabled and state != "wall_riding":
...
func handle_wall_ride(delta: float, input_dir: Vector2):
	if is_on_floor():
		wall_jumped = false
		if state == "wall_riding":
			exit_wall_ride_state()
		return

	if state == "wall_riding":
		wall_ride_timer -= delta

		if wall_ride_timer <= 0.0 or not is_on_wall():
			exit_wall_ride_state()
			return

		wall_normal = get_wall_normal()

		# Steer along wall — input steers, no input coasts with light friction
		if input_dir:
			var dir2 := input_dir.rotated(-HEAD.rotation.y)
			var dir3 := Vector3(dir2.x, 0, dir2.y)
			var along_wall := dir3 - dir3.dot(wall_normal) * wall_normal
			along_wall.y = 0
			var along_wall_n := along_wall.normalized()
			velocity.x = lerp(velocity.x, along_wall_n.x * speed, acceleration * delta)
			velocity.z = lerp(velocity.z, along_wall_n.z * speed, acceleration * delta)
		else:
			velocity.x = lerp(velocity.x, 0.0, 3.0 * delta)
			velocity.z = lerp(velocity.z, 0.0, 3.0 * delta)

		# Wall push AFTER lerp so it isn't lerped away (consumed by collision in move_and_slide next frame)
		velocity -= wall_normal * 4.0
		velocity.y = 0.0

		print("[WallRide] vel=", Vector3(snappedf(velocity.x,0.01), snappedf(velocity.y,0.01), snappedf(velocity.z,0.01)), " | xz_speed=", snappedf(Vector2(velocity.x, velocity.z).length(), 0.01))

		# Wall jump — launch in look direction
		if jumping_enabled and Input.is_action_just_pressed(controls.JUMP):
			var cam_fwd := -CAMERA.global_transform.basis.z
			cam_fwd.y = 0.0
			var jump_dir := (cam_fwd.normalized() + Vector3.UP).normalized()
	...
	# Stick to wall when airborne — no input required, just touch it
	if wall_jumped or state == "crouching" or not is_on_wall():
		return
	if stamina_enabled and stamina <= 0.0:
		return

	enter_wall_ride_state(get_wall_normal())


func enter_wall_ride_state(normal: Vector3):
	state = "wall_riding"
	wall_normal = normal
	wall_ride_timer = wall_ride_duration
	speed = base_speed
	# Strip normal component first, then boost only the true along-wall velocity
	var hv := Vector3(velocity.x, 0.0, velocity.z)
	var along_wall := hv - hv.dot(normal) * normal
	velocity.x = along_wall.x * 1.5
	velocity.z = along_wall.z * 1.5
	velocity -= normal * 4.0
	velocity.y = 0.0
	print("[WallRide] START — pos=", global_position, " | wall_normal=", normal, " | vel=", velocity)


func exit_wall_ride_state():
	state = "normal"
	speed = base_speed
	print("[WallRide] END — pos=", global_position, " | vel=", velocity)


func handle_lean(delta: float):
	if not lean_enabled or state == "wall_riding":
		lean_target = 0.0
	else:
		var lean_input := 0.0
		if InputMap.has_action(controls.LEAN_LEFT) and Input.is_action_pressed(controls.LEAN_LEFT):
			lean_input -= 1.0
		if InputMap.has_action(controls.LEAN_RIGHT) and Input.is_action_pressed(controls.LEAN_RIGHT):
			lean_input += 1.0
		lean_target = lean_input * deg_to_rad(lean_angle)
	CAMERA.rotation.z = lerp(CAMERA.rotation.z, lean_target, lean_speed * delta)
...
func handle_stamina(delta: float):
	...
	if state == "wall_riding":
		stamina = max(0.0, stamina - stamina_drain_wall_ride * delta)
		draining = true
		if stamina <= 0.0:
			exit_wall_ride_state()
...
func handle_state(moving):
	if state == "wall_riding":
		return
	if sprint_enabled:
		var can_sprint := not (stamina_enabled and stamina <= 0.0)
		if sprint_mode == 0:
			...
			if moving:
				...
				if Input.is_action_pressed(controls.SPRINT) and state == "normal" and can_sprint:
					enter_sprint_state()
...
func update_debug_menu_per_tick():
	...
	if stamina_enabled:
		$UserInterface/DebugPanel.add_property("Stamina", snappedf(stamina, 0.1), 5)
